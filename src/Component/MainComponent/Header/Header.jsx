...
import ComplainOptions from "./ComplainOptions";
// import UserPaymentOptionsModel from "../../Transaction/UserPaymentOptions/UserPaymentOptionsModel";
// import UserComplainOptions from "./UserComplainOptions";
...
export default function Header() {
  ...
  const dueDate = new Date(organisation?.duepaymentdate);
  // useEffect(() => {
  ...
  const [usercomplainOpen, setUserComplainOpen] = useState(false);

  const handleUserComplainOpen = () => {
    setUserComplainOpen(true);
  };

  const handleUserComplainClose = () => {
    setUserComplainOpen(false);
  };
  const {
    data: yeardata,
    loading: yearloading,
  ...
  useEffect(() => {
    // console.log(locationdata,"locationdata",locationdata[0].tlngval,locationdata[0].tlatval);
    const fetchWeather = async () => {
      try {
        const response = await axios.get(
          ...
          {
            params: {
              // lat: locationdata[0]?.tlatval || "31.559267",
              // lon: locationdata[0]?.tlngval || "74.321762",
              lat: "31.559267",
              lon: "74.321762",
              appid: "82df9ca72110e4d9021535d7cb1957e5",
              units: "metric",
            },
          }
        );
          ...
          <Toolbar>
            ...
            {showDetails && (
              <div
                ...
                onClick={() => setShowDetails(false)} // click outside to close
              >
                <div
                  style={{
                    background: getnavbarbackgroundcolor,
                ...
                >
                  ...
                  <p>
                    <strong>Temperature:</strong> {weather.main.temp}°C (Feels
                    like {weather.main.feels_like}°C)
                  </p>
                  <p>
                    <strong>Min / Max Temp:</strong> {weather.main.temp_min}°C /{" "}
  ...
  `}
              ...
              {organisation && organisation.code === "CRYSTAL" && (
                <>
                  <IconButton
                    ...
                    onClick={() => navigate("/AdminComplainOptions")}
                  >
                    <i
                      className="bi bi-exclamation-triangle fs-5"
                      style={{ color: "#ffffff" }}
                    ></i>
                  </IconButton>
                  <IconButton
                    ...
                    onClick={() => navigate("/AdminCollectionOptions")}
                  >
                    <i
              ...
              <IconButton
                ...
                onClick={() => navigate("/UserComplainOptions")}
              >
                <i
                  className="bi bi-exclamation-triangle fs-5"
                  style={{ color: "#ffffff" }}
              ...
              <IconButton
                ...
                onClick={() => navigate("/UserPaymentOptions")}
              >
                <i
                  className="bi bi-credit-card fs-5"
                  style={{ color: "#ffffff" }}
        ...
        <PaymentOptions open={paymentOpen} onClose={handlePaymentClose} />
        <ComplainOptions open={complainOpen} onClose={handleComplainClose} />
        {/* <UserComplainOptions
        open={usercomplainOpen}
        onClose={handleUserComplainClose}
      /> */}

        {renderMobileMenu}
        {renderMenu}
      </Box>
      {/* <UserPaymentOptionsModel show={showPaymentModal} onHide={handleReturn} /> */}
    </>
  );
}
