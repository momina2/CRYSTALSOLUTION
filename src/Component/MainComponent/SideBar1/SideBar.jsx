...
//         <List
//           className="custom-sidebar-file overflow-y-auto overflow-x-hidden rounded-xl flex-grow" // flex-grow added to make it take remaining space
//           style={{ padding: "8px 4px" }} // Reduced padding for compact look
//         >
...
const SideBar1 = () => {
  ...
  // =============
  // Sub-Sub-Items (فاصلہ کم کیا گیا اور بارڈر گہرا کیا گیا)
  // =============
  const renderSubSubMenu = (subItems) => {
    const filteredSubItems = subItems.slice(1); 
    ...
    return filteredSubItems.map(
      (subItem, index) => (
        <ListItem
          ...
          disabled={subItem.disabledd}
          // height اور margin کم کی گئی
          className={`pl-14 h-7 rounded-md my-[0.05rem] transition-colors duration-200 
            ${getcolor === 'white' ? 'hover:bg-gray-100' : 'hover:bg-white/5'} 
            ${subItem.disabledd ? 'opacity-50' : ''}`}
          sx={{ 
            pl: 5.5, 
            height: "28px", // Height کم
            my: 0, // Margin کم
            // Divider line کو گہرا کیا گیا
            borderBottom: (index < filteredSubItems.length - 1) 
              ? `1px solid ${getcolor === 'white' ? '#e0e0e0' : 'rgba(255, 255, 255, 0.15)'}` 
              : 'none',
          }}
        ...
        <React.Fragment key={middleLevel}>
          ...
          {hasSubSubMenu && (
            <Collapse
              ...
              className={`rounded-lg mx-1 mb-1 p-0.5`}
              sx={{
                // یہ Collapse Middle-Level Item کے Sub-Sub Items کو دکھاتا ہے۔
                backgroundColor: getcolor === 'white' ? '#fafafa' : 'rgba(0,0,0,0.1)', 
              }}
            >
              <List component="div" disablePadding sx={{ py: 0 }}>
          ...
          {Object.keys(hierarchicalMenuData)
            ...
            .map((topLevel) => {
              ...
              <React.Fragment key={topLevel}>
                ...
                {/* Top-Level Collapse - یہ پورا مینو بلاک ہے۔ */}
                <Collapse
                  in={isActive} 
                  timeout="auto"
                  unmountOnExit
                  ...
                  sx={{ 
                    padding: "4px 0",
                    // --- 1. Top-Level Menu کو Block سٹائل دینا ---
                ...
                >
                  {/* List میں معمولی پیڈنگ تاکہ بلاک کے اندر جگہ بنے */}
