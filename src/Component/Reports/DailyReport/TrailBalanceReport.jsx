...
const REPORT_CONFIG = {
  ...
  TABLE: {
    ...
    KEYS: ["Code", "Description", "Opening", "Debit", "Credit", "Closing", ""],
    UI_WIDTHS: {
      col001: "80px",
      col002: "270px",
      col003: "120px",
    ...
    PDF_WIDTHS: [22, 72, 28, 28, 28, 28, 0],
    EXCEL_WIDTHS: [12, 40, 16, 16, 16, 16, 0],
    ALIGNMENTS: ["left", "left", "right", "right", "right", "right", ""],
    PDF_ALIGNMENTS: ["left", "left", "right", "right", "right", "right", ""],
...
export default function TrailBalanceReport() {
  ...
  // State variables for all API provided totals
  const [totalCollection, setTotalCollection] = useState("0.00"); // Used for ClosingBal
  const [totalOpening, setTotalOpening] = useState("0.00"); // New for OpeningBal
  const [totalDebit, setTotalDebit] = useState("0.00"); // New for TotalDebit
  const [totalCredit, setTotalCredit] = useState("0.00"); // New for TotalCredit
  ...
  function fetchDailyMemberCollectionDetailReport() {
    ...
    const formMainData = new URLSearchParams({
      [API_PARAMS.ORGANISATION_CODE]: 'ZSTRD',
    ...
    // REMOVED: Helper function to convert a formatted financial string 
    // to a number (was used for client-side total calculation)
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data && Array.isArray(response.data.Detail)) {
          ...
          // FIX: Use API provided totals as requested by the user
          // Note the keys from the API response example (some have trailing spaces):
          const apiOpeningBal = String(response.data["OpeningBal "] || "0.00");
          const apiTotalDebit = String(response.data["TotalDebit "] || "0.00");
          const apiTotalCredit = String(response.data["TotalCredit"] || "0.00");
          const apiClosingBal = String(response.data["ClosingBal "] || "0.00");

          setTotalOpening(apiOpeningBal);
          setTotalDebit(apiTotalDebit);
          setTotalCredit(apiTotalCredit);
          setTotalCollection(apiClosingBal); // totalCollection is used for ClosingBal

        } else {
          setTableData([]);
          setTotalOpening("0.00");
          setTotalDebit("0.00");
          setTotalCredit("0.00");
          setTotalCollection("0.00");
        }
      })
      .catch((error) => {
  ...
  const exportPDFHandler = () => {
    ...
    totalRowData[2] = String(totalOpening); // Opening
    totalRowData[3] = String(totalDebit);   // Debit
    totalRowData[4] = String(totalCredit);  // Credit
    totalRowData[5] = String(totalCollection); // Closing
  ...
  const handleDownloadCSV = async () => {
    ...
    totalRowData[0] = "Total";
    totalRowData[2] = String(totalOpening);
    totalRowData[3] = String(totalDebit);
    totalRowData[4] = String(totalCredit);
    totalRowData[5] = String(totalCollection);
    ...
    <>
      <style>
        {`
          ...
          */
          ...
          /* New style for visible scrollbar */
          .table-scroll {
            overflow-y: auto; /* Ensure scrollbar is available when content overflows */
            overflow-x: hidden;
            -ms-overflow-style: auto;  /* IE and Edge - Reset to default */
            scrollbar-width: auto;  /* Firefox - Reset to default */
          }
        `}
      </style>
      <ToastContainer />
      <div style={contentStyle}>
        ...
        >
          ...
          >
            {columnsConfig.map((column, index) => {
              // Map the column index to the correct API total state
              let totalValue = "";
              let isTotalColumn = false;
              
              if (index === 2) { // Opening column index
                totalValue = totalOpening;
                isTotalColumn = true;
              } else if (index === 3) { // Debit column index
                totalValue = totalDebit;
                isTotalColumn = true;
              } else if (index === 4) { // Credit column index
                totalValue = totalCredit;
                isTotalColumn = true;
              } else if (index === 5) { // Closing column index
                totalValue = totalCollection; // totalCollection holds ClosingBal
                isTotalColumn = true;
              }
