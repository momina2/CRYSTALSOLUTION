...
const REPORT_NAME = `American Purchase Report ${lastYear}`;
...
export default function AmericanPurchaseReportLastYear() {
  const navigate = useNavigate();
  const [rows, setRows] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [fromDate, setFromDate] = useState(firstDayOfLastYear);
  const [toDate, setToDate] = useState(lastDayOfLastYear);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.post(
        "https://crystalsolutions.pk/api/ItemPurchaseSummary.php",
        form,
        { timeout: 20000 },
      );
  ...
  useEffect(() => {
    setFromDate(firstDayOfLastYear);
    setToDate(lastDayOfLastYear);
  }, []);
  ...
  const exportPDFHandler = () => {
    ...
    totalRow[keys.length - 1] = totalPurchaseAmount.toLocaleString();
  ...
  const sortedTableData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(
        (row) =>
          ...
          row["Pur Amount"]?.toString().includes(q),
      );
    }
    ...
    if (sortConfig.key) {
      data.sort((a, b) => {
        ...
        // 🔢 Numeric columns
        if (["Qnty", "Rate", "Pur Amount"].includes(sortConfig.key)) {
  ...
  const totalPurchaseAmount = useMemo(() => {
    return filteredData.reduce((sum, row) => {
      const val = Number(String(row["Pur Amount"] || "0").replace(/,/g, ""));
      return sum + (isNaN(val) ? 0 : val);
    }, 0);
  }, [filteredData]);
  ...
  const handleCSV = () => {
    exportCSV({
      ...
      columnsConfig,
      totalCollection: totalPurchaseAmount,
