...
// Example:
// February 2026
const REPORT_NAME = `American Collection Report - ${currentMonthName} ${currentYear}`;
...
export default function AmericanCollectionCurrentMonth() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.post(
        "https://crystalsolutions.pk/api/AmericanCollectionReport.php",
        form,
        { timeout: 20000 },
      );
  ...
  const sortedTableData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(
        (row) =>
          ...
          row.Collection?.toString().includes(q),
      );
    }
    ...
    if (sortConfig.key) {
      data.sort((a, b) => {
        ...
        // 🔢 Numeric columns
        if (["Collection"].includes(sortConfig.key)) {
  ...
  const handleCSV = () => {
    exportCSV({
      ...
      columnsConfig,
      totalCollection: apiTotalAmount, // ✅ API TOTAL
