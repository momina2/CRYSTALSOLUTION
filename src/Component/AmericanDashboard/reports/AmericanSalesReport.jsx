...
export default function AmericanSalesReport() {
  ...
  const fetchCustomer = async () => {
    try {
      ...
      const res = await axios.post(
        "https://crystalsolutions.pk/api/GetCustomers.php",
        form,
      );
  ...
  const exportPDFHandler = () => {
    ...
    totalRow[keys.length - 1] = totalAmount.toLocaleString();
  ...
  const sortedTableData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(
        (row) =>
          row.titmcod?.toLowerCase().includes(q) ||
          row.ttrndsc?.toLowerCase().includes(q) ||
    ...
    if (sortConfig.key) {
      data.sort((a, b) => {
        ...
        // 🔢 Numeric columns
        if (["Qnty", "Amount"].includes(sortConfig.key)) {
  ...
  const filteredData = useMemo(() => {
    ...
    return sortedTableData.filter((row) => {
      return (
        row.titmcod?.toLowerCase().includes(q) ||
        row.ttrndsc?.trim().toLowerCase().includes(q) || // ✅ NAME FIX
        row.Qnty?.toString().includes(q) || // ✅ Balance
        row.Amount?.toString().includes(q) // ✅ Balance
      );
    });
  ...
  const totalAmount = useMemo(() => {
    return filteredData.reduce((sum, row) => {
      const val = Number(String(row.Amount || "0").replace(/,/g, ""));
      return sum + (isNaN(val) ? 0 : val);
    }, 0);
  }, [filteredData]);
              ...
              >
                ...
                >
                  ...
                  {Array.isArray(customers) &&
                    customers.map((c) => (
                      <option key={c.id} value={c.tcstcod}>
                        {c.tcstcod} - {c.tcstdsc.trim()}
                      </option>
                    ))}
                </select>

                {/* COMPANY CODE (future API) */}
                <select
                  value={company}
                ...
                >
                  ...
                  {Array.isArray(category) &&
                    category.map((c) => (
                      <option key={c.id} value={c.tctgcod}>
